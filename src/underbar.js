(function() {
  ...
  _.memoize = function(func) {
    ...
    return function(args){
      if (previous[args] === undefined){
        previous[args] = func.apply(this, arguments);
      }
  ...
  _.shuffle = function(array) {
    ...
    var positions = [];
    var newArr = [];
   //generate array of new positions
    while (positions.length<arr.length){
      var value = Math.floor((Math.random() * len));
      if (_.indexOf(positions,value) === -1){
        positions.push(value);
      }
    }
    //push values into new array based on positions
    for (var i = 0; i<len; i++){
      newArr.push(arr[positions[i]]);
    }
    return newArr;
  };
  ...
  _.zip = function() {
    ...
    var result = [];
    for (var i = 0; i < numArrays; i++) if (longestLength < arguments[i].length) longestlength = arguments[i].length;
    for (var i = 0; i < longestLength; i++){
      var indexArray = [];
      for (var k = 0; k < numArrays; k++){
        indexArray.push(arguments[k][i]);
      }
  ...
  _.intersection = function() {
    var temp = _.uniq(arugments[0]);
    for (var i = 1; i<arguments.length; i++){
      var arg = _.uniq(arguments[i]);
      for (var k = 0; k<temp.length; k++){
        var value = temp[k]
        var index = _.indexOf(arg,value);
        if (index === -1) temp[index] = null;
      }
    }
    var output = [];
    for (var i=0; i<temp.length; i++){
      if (temp[i] !== null) output.push(temp[i]);
    }
    return output;
  };
